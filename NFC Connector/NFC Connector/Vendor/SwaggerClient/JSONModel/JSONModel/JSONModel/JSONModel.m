...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (dict != nil) {
        ...
        id cycle_tag = [dict valueForKey:@"cycle"];
        if (cycle_tag != nil && [cycle_tag isKindOfClass:[NSDictionary class]]) {
            NSLog(@"cycle Dictionary Found and notification generated");
            [[NSNotificationCenter defaultCenter] postNotificationName:@"cycleReceived" object:cycle_tag];
        } else if (cycle_tag != nil && [cycle_tag isKindOfClass:[NSArray class]]) {
            NSArray* cycleArray = (NSArray*) cycle_tag;
            if ([cycleArray count]>0) {
                id cObject = [cycleArray objectAtIndex:0];
                if (cObject != nil && [cObject isKindOfClass:[NSDictionary class]]) {
                    NSLog(@"cycle Dictionary Found and notification generated");
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"cycleReceived" object:cObject];
                }
            }
        }

        id tag = [dict valueForKey:@"tag"];
        if (tag != nil && [tag isKindOfClass:[NSArray class]]) {
            NSArray* tagArray = (NSArray*) tag;
            if ([tagArray count]>0) {
                id tagObject = [tagArray objectAtIndex:0];
                if (tagObject != nil && [tagObject isKindOfClass:[NSDictionary class]]) {
                    dict = tagObject;
                }
            }

        } else if (tag != nil && [tag isKindOfClass:[NSDictionary class]]) {
            dict = tag;
        }
    }
