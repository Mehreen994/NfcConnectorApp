//
//  UseMedicalProductViewController.m
//  NFC Connector
//
...
@interface UseMedicalProductViewController () <LocationProtocolDelegate> {
    CLLocationCoordinate2D currentLocation;
}
@end

@implementation UseMedicalProductViewController
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([sender isEqualToString:SHOW_PRODUCT_DATA]) {
        ...
        destination.applicationType = medical;
...
-(void) onLocationFound:(CLLocationCoordinate2D)coordinate calledBy:(nonnull NSString *)call tag:(nonnull NSString *)tag_id {
    if ([call isEqualToString:REGISTER_CYCLE] ||
        [call isEqualToString:REGISTER_STER_CYCLE] ||
        [call isEqualToString:SHOW_CONNECTOR_DATA] ||
        [call isEqualToString:SHOW_PRODUCT_DATA]) {
        NSLog(@"UseMedical onLocationFound ****** coord = %f  long %f", coordinate.latitude, coordinate.longitude);
